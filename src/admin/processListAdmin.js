    ...
    useEffect(()=>{
        ...
        for(let btn of btn_delete){
            btn.addEventListener("click", function(){
                ...
                axios.delete(`https://s2qbne-8080.preview.csb.app/api/APIprocess/${id}`)
                .then(()=> {
        ...
        ${process.map((item, index)=>{
            ...
            <tbody>
                <tr class="text-center">
                    ...
                    <td class="border px-2 text-[#ffff] text-justify">${item.description}</td>
                    <td class="border px-2 text-[#ffff]">
                        <button data-id="${item.id}" class="bg-red-600 p-5 py-2 text-[#ffff] btn_delete">DELETE</button>
                        <a href="/admin/processEditAdmin/${item.id}" class="bg-blue-900 p-5 py-[11px] text-[#ffff]">SETTING</a>
                    </td>
                </tr>
            </tbody>
            `
